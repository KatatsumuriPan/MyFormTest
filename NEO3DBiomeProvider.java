...
public final class NEO3DBiomeProvider {
        ...
        for (int i = 0; i < iterates; i++) {
            ...
            for (int j = 0; j < biomeCenterList.size(); j++) {
                ...
                if (p1.y > 255 - spaceHeight - 10 - biomeSize.getY() / 2)
                    v1.y -= (p1.y - (255 - spaceHeight - 10 - biomeSize.getY() / 2)) * 0.5;
            ...
            for (int j = 0; j < biomeCenterList.size(); j++) {
                ...
                p1.y = MathHelper.clamp(p1.y, magmaHeight + 1, areaSize.getY() - spaceHeight - 1);
        ...
        for (int y = 0; y < areaSize.getY(); y++) {
            for (int z = 0; z < areaSize.getZ(); z++) {
                for (int x = 0; x < areaSize.getX(); x++) {
                    ...
                    // 宇宙
                    {
                        float dis = (255 - y) * (255 - y) + (x % 16) * (16 - x % 16) * rx + (z % 16) * (16 - z % 16) * rz;
                        if (dis < minDisSq) {
                            minDisSq = dis;
    ...
    public byte getBiome(int x, int y, int z) {
        return (byte) (biomeIndicies[xyz2Index(x, y, z)] & 0x7f);
    }

    public boolean isEdge(int x, int y, int z) {
        return (biomeIndicies[xyz2Index(x, y, z)] & 0x80) != 0;
    }
